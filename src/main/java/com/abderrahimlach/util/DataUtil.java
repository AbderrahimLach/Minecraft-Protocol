package com.abderrahimlach.util;
...
public class DataUtil {
    ...
    public static int readUnsignedVarInt(DataInput in) throws IOException {
        ...
        while (((data = in.readByte()) & 0x80) != 0) { // check if MSB is set. When this equals 0 we have reached the last byte
            ...
            if (iterations > 5) {
                MinecraftServer.getInstance().warn("Too many bits received while reading unsigned var int");
                return -1;
            }
        }
    ...
    public static int readUnsignedVarInt(ByteReader reader) {
        ...
        while (((data = reader.next()) & 0x80) != 0) {
            ...
            if (iterations > 5) {
                ...
                MinecraftServer.getInstance().warn("Too few bits received while reading unsigned var int");
            }
        }
    ...
    public static String readString(ByteReader reader) throws IllegalArgumentException {
        ...
        for (int i = 0; i < bytes; i++) {
            if (reader.hasNext()) {
                ...
                MinecraftServer.getInstance().warn("Too few bytes here");
